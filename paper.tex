...
A $p$-value for testing model $M_{k-1}$ against $M_k$ satisfies the (conditional) {\em sufficiency principle} if, on the event $\{M_{k-1}=m_{k-1}, M_k=m_k\}$, it depends only on sufficient statistics of $M_k$. 
...
If $T(Y)$ is a complete sufficient statistic, then any exact $p$-value must have the latter property.
