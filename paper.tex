...
\begin{abstract}
  Many model selection algorithms produce a ``path'' of fits specifying a sequence of models of increasing complexity. Given such a sequence and the data used to produce them, we consider the problem of choosing the least complex model that is not falsified by the data, while accounting for the fact that the model path is determined adaptively using the data. Extending the selected-model tests of \citet{fithian2014optimal}, we construct $p$-values for each step in the path. In the case of linear regression, our $p$-values improve on the power of the spacings test of \citet{taylor2014exact}. The improvement in power can be dramatic for ``early steps'' in the path, when most of the signal has not yet been added to the model. To choose a model we apply stopping rules to the resulting $p$-values, achieving adaptive control of the familywise error rate and false discovery rate.

Most of the stopping rules proposed in the literature assume that the single-step null $p$-values are independent of each other and of the non-null $p$-values. We derive intuitive and general conditions for independence and show that our proposed construction yields independent $p$-values.
\end{abstract}
...
\WFcomment{References.} \citet{fithian2014optimal} call $M_\infty$ the {\em saturated model} because there is a parameter for every data point. They contrast tests based on $M_\infty$ with {\em selected-model} tests, which do not treat $\proj_\eta^\perp \mu$ as a nuisance parameter. 
