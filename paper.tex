...
\end{equation}
where $X_E$ is the matrix with columns $(X_j)_{j\in E}$. Because $U_E=\left(X_E'Y,\, \|Y\|^2\right)$ is the complete sufficient statistic for $M(E)$, we can sample from the conditional null distribution of $t_E^*$ given $U_E$ and the current active set $E$, using the {\em selected-model test} framework of \citet{fithian2014optimal}. In step one, because $E=\emptyset$ is fixed and $t_E^*$ is independent of $U_E$, the conditional test reduces to the max-$t$ test proposed in \citet{buja2014}. In later steps, $t_E^*$ and $U_E$ are not conditionally independent given $E$, but we can approximate the null conditional distribution through Monte Carlo sampling.
...
Assuming for simplicity that there is a unique solution to each penalized problem of the form
\begin{equation}\label{eq:pen-lik}
  \hat\theta^{\lambda}(Y) =
  \argmin_{\theta\in\Theta} -\ell(\theta; Y) + P_\lambda(\theta),
\end{equation}
we define the ever-active set for $\lambda\in\Lambda$ as
\begin{equation}
  \tE_\lambda(Y) = \left\{j:\; \hat\theta_j^\nu(Y) \neq 0 
    \text{ for any } \nu\in \Lambda, \nu \geq \lambda \right\}
\end{equation}
It may be impossible or inconvenient to compute $\hat\theta^\lambda(Y)$ for every $\lambda\in(0,\infty]$. If so, we can instead take $\Lambda$ to be a grid of finitely many values.
...
\JTcomment{The next-$\lambda$ test is actually a little expensive to compute if we a finite set of $\lambda$  as, say, glmnet does. Should we not mention this? On the plus side, 
...
value of $\lambda$. Then we compare our observed next-$\lambda$ to this
sample of $\lambda$'s.

In the LAR-LASSO case, this is easier as there is an explicit formula
to compute the next event. But, LAR is limited to LASSO whereas next-$\lambda$
...
next-$\lambda$ is essentially only practical BLAH
}
...
As a result the lasso coefficients $\hat\beta_{\text{lasso}}^\lambda$ minimize
\[
\exp\{-2\ell(\beta,\sigma;\,Y)/n \,+\, \text{const.}\} + \lambda\|\beta\|_1,
\]
instead of the criterion in \eqref{eq:pen-lik}. If $\Lambda=(0,\infty)$ then both methods result in exactly the same ever-active path. Otherwise, we could ``fix'' this issue (at the cost of some notational bloat) by replacing $-\ell(\cdot)$ with $\rho(\ell(\cdot))$, where $\rho$ is any strictly decreasing function, and all of the theory in this article would go through unchanged.
\end{comment}
...
These are the true ``partial regression coefficients''  for design matrix $X_E$.
 Write $\proj_E\mu = X_E \theta^E$ for the projection of $\mu$ into the column space of $X_E$.
...
In our problem setting, the sequence of selected models is random; thus, the truth or falsehood of each $H_k$ is not fixed, as assumed in the previous section. It is not immediately clear, then, how much we should condition on when we construct the $p$-value at step $k$. In particular, is it sufficient to condition only on the current null model $M_{k-1}$, or must we condition on the entire subpath of models $M_0,\ldots,M_{k-1}$?

Note that conditioning on the current null model {\em is} enough to guarantee that $p_k$ is uniform conditional on the event $\{F\in M_{k-1}(Y)\}$ that the $k$th null is true. Unfortunately, however, it is {\em not} enough to guarantee that the above stopping rules actually control FDR or FWER. 

Let $z_{\alpha/2}$ denote the upper-$\alpha/2$ quantile of the $\cN(0,1)$ distribution and consider the following counterexample:

\begin{restatable}{proposition}{counterex}\label{prop:counterexample}
  Consider linear regression with $n=p=3$, known $\sigma^2=1$ and identity design $X=I_3$.

  Suppose that we construct the model path as follows: if $|Y_3|>z_{\alpha/2}$, we add $X_1$ to the active set first, then $X_2$, then $X_3$. Otherwise, we add $X_2$, then $X_1$, then $X_3$. At each step we construct selective max-$z$ $p$-values and finally choose $\hat k$ using BasicStop. 

  If $\mu=(0,C,0)$, then the FWER for the entire procedure tends to $2\alpha-\alpha^2$ as $C\to\infty$.
\end{restatable}
