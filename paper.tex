...
Due to the generality in which we have posed the problem here, we cannot give a general prescription for how to carry out this sampling. However, we can go into some detail for the important case of linear regression.

\subsection{Details for Linear Regression}
...
We focus here on the simpler case where $\sigma^2$ is known. For $\sigma^2$ unknown, see \citet{fithian2014optimal}. If $\sigma^2$ is known, then $U_{k-1}(Y) = X_{E}'Y$ on $A$, so our task is to sample under the null from
\[
\L(Y \mid X_E'Y, Y\in A),
\]
a truncated multivariate Gaussian distribution.
...
\paragraph{Accept-Reject Sampling:}
Note that sampling from $\L(Y \mid X_E'Y)$ is easy. Letting $\proj_{X_E}$ denote projection into the column space of $X_E$, we can decompose $Y$ into two independent terms:
\[
Y = \proj_{X_E}Y + \proj_{X_E}\perp Y.
\]
The first term is fixed conditional on $X_E'Y$, while the second is a degenerate Gaussian distribution with mean zero and covariance matrix $\sigma^2\proj_{X_E}^\perp$.



For a fixed set $E$, we must be able to ample from 

we must be able to sample from the event on the event $E_{k-1}(Y)=E$,


 if $\sigma^2$ is known, and $(X_{E_{k-1}}'Y, \|Y\|^2)$ otherwise. Thus for a fixed active


 In either case, sampling from $\L(Y \mid U_{k-1})$ is easy under the null hypothesis that $\E Y$ lies in the column span of $X_{E_{k-1}}$. Write
\[
Y = \proj_{E_{k-1}} 
\]
...
, and we need to sample under the null hypothesis that $Y$ is marginally Gaussian, with $\E Y$ in the column span of $X_{E_{k-1}}$.





: if $\sigma^2$ is known, then $Y$ 


but sampling from $\L(Y \mid E_{k-1}, \, U_{k-1})$ . We focus first on the casIf $\sigma^2$ is known, we can sample


 therefore not a given $U_{k-1}$ is n

The event that $M_{k-1}(Y)$

\subsection{Linear Regression}



See \citet{fithian2014optimal}



 Instead, we give details of our computational strategy here for one case, the max-$z$ test in forward stepwise regression with known $\sigma^2$.
