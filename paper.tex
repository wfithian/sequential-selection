...
\author{William Fithian, Jonathan Taylor, Rob Tibshirani, and Ryan J. Tibshirani} 
\maketitle
...
\[
M_0 \sub M_1(Y) \sub \cdots \sub M_d(Y) \sub M_\infty.
\]
...
\[
E_0 \sub E_1(Y) \sub \cdots \sub E_d(Y) \sub \{1,\ldots J\},
\]
\RJTcomment{Again here I've removed the dependence of $E_0$ on $Y$.} \\
...
Another important class of model selection procedures is the sequence of {\em ever-active} sets for a regularized likelihood path, under a sparsity inducing regularizer. The notion of ever-active sets is needed since these solution paths can drop  (as well as add) predictors along the way. 	For some ordered set $\Lambda$, let $\lambda\in\Lambda$ parameterize the regularization penalty $P_\lambda(\theta)$. As an example, for a lasso penalty, this is $P_\lambda = \lambda\|\beta\|_1$ with $\lambda\in (0,\infty)$. 
...
\RJTcomment{Hopefully without too much drama :), I've added the words ``sparsity-inducing regularizer'' above.  This is because, at least as we've written it, we are thinking of the regularizer as selecting out a submodel.  I understand that it may be more general than this, but our notation above and in the previous subsection all have to do with sparsity directly.}
...
\[
\hat\beta_{\text{lasso}}^\lambda = \argmin_{\beta\in\R^p} \; \frac{1}{2}\|Y-X\beta\|_2^2 + \lambda\|\beta\|_1, 
\]
...
\RJTcomment{I actually find this last remark more confusing than helpful, and I think we should take it out.  I think it's pretty obvious that you can transform the loss by a monotone function ... and what is the $\exp$ doing up there?  As it's written that's not a log-likelihood, that's a likelihood?} 
...
Fix a function $h: [0,1] \rightarrow [0,\infty]$ satisfying \smash{$\int_{t=0}^1 h(t)dt=1$}, called an {\em accumulation function.} Using $h$ we can construct an estimate of FDP at step $k$:
\begin{equation*}
\widehat{\text{FDP}}_h(k)= \frac{\sum_{i=1}^k h(p_i)}{k},
\end{equation*}
and select the largest model for which \smash{$\widehat{\text{FDP}}_k \leq \alpha$}. Taking $h(p)=-\log(1-p)$ recovers the ForwardStop rule. Another choice they propose is the {\em HingeExp} function
\begin{equation*}
  h_{\rm HingeExp}(p)= 
  \begin{cases} 
...
\]
then BasicStop again would control FWER, by the same argument as given in \eqref{eq:basic-stop-control}. Note that $k_0$ is unknown, so we cannot directly condition on its value when we construct our single-step $p$-values. However, because
\[
\{k_0=k\} = \{F\in M_{k-1}, F\notin M_{k-2}\},
\]
...
Next we will explain why $p_1,\ldots,p_{k-1}$ are functions of $E_{k-1}$ and $U_{k-1}$. Knowing $E_{k-1}=E$ tells us that the $k-1$ variables in $E$ are selected first, and knowing \smash{$(X_{E_{k-1}}'Y,\|Y\|^2)$} is enough information to compute the $t$-statistics $t_{j,D}$ for all $j\in E$ and $D\sub E$. As a result, we can reconstruct the order in which those $k-1$ variables were added, since for $i<k$, 
\[
...
\RJTcomment{A thought / suggestion: we define the SSP here, but I think it's not really that helpful, because we're only talking about max-t.  We refer to SSP in the next section; in defining the sufficient filtration.  Why not just define SSP there?  In general, I think there's quite a lot of lingo being thrown around, and though I like all the new terms, I think we could do a little better job consolidating where they're defined.}
...
\end{enumerate}
If we think of $\sF_k$ as representing information available at step $k$, then the first condition means that $p_k$ ``excludes'' whatever evidence may have accrued against the null by step $k-1$, and the second means that any information revealed after step $k$ plays no role in determining $p_k$. Separated $p$-values are independent on nulls, as we see next.
...
\end{enumerate}
To be a valid selective $p$-value for a single step, $p_k$ only needs to be super-uniform conditional on $M_{k-1}$. In general, it is more stringent to additionally require that $p_k$ must condition on the entire subpath $M_{0:(k-1)}$ as well as $U_{k-1}$. In most cases of interest, however, (i) we will want to condition on $(M_{0:(k-1)}, U_{k-1})$ anyway, to eliminate nuisance parameters of $M_{k-1}$, and (ii) the path satisifies the SSP, so conditioning on $(M_{0:(k-1)}, U_{k-1})$ is equivalent to conditioning on $(M_{k-1}, U_{k-1})$. Thus, most single-step selective $p$-values we would contemplate using are already exact conditional on $\sF_{k-1}$. The second requirement, that $p_k$ be $\sF_k$-measurable, has more bite. For example, we will see in Section~\ref{sec:selective-reg} that it excludes saturated-model tests in linear regression.
...
In this section, we apply Theorem~\ref{thm:suffCond} to establish that in the generic sparse parametric model of Section~\ref{sec:genSparse}, the forward stepwise path and all ever-active regularization paths satisfy the SSP. Moreover, the greedy likelihood ratio statistic $G_k$ is $\sF_k$-measurable with respect to the sufficient filtration of the forward stepwise path, and the next-entry statistic $\lambda_k$ is $\sF_k$-measurable with respect to the sufficient filtration of the regularized-likelihood path. As a result, the selective greedy likelihood $p$-values in the forward stepwise path are independent on nulls, as are the next-entry $p$-values in the regularized-likelihood path, per Theorem~\ref{thm:suffCond}.
...
We begin by stating our result for forward stepwise.
\begin{proposition}\label{prop:forwardSSP}
Forward stepwise paths satisfy the subpath sufficiency principle.
\end{proposition}
\begin{proof}
...
The analogous result hold for ever-active regularized likelihood paths.
...
\begin{proof}
  Again, for fixed $k$ and $E$, let $A=\{M_k = M(E)\}$, and define $U=U(Y;\, M(E))$. If we denote 
\[
...
\begin{proof}
  ...
  E_k = \bigcap_{\lambda\in\Lambda_k} \tE_{\lambda},
  \]
  and that the \smash{$\tE_{\lambda}$} are nested by construction and finite by assumption. As a result, there is some $\nu_k\in \Lambda_k$ for which \smash{$\tE_{\nu_k} = E_k$}. 
...
Many conditional selective tests for linear regression use the saturated-model framework. See for example \citet{lockhart2014significance}, \citet{taylor2013tests}, \citet{taylor2014exact}, \citet{lee2013exact}, and \citet{loftus2014significance}. 
\RJTcomment{So, I would claim that the covariance test is NOT a saturated model test, but instead a select model test.  Of course it doesn't really fit into this framework with exact p-values, but if you look at what we proved, it was a selected model null result.  Also, Taylor (2013) tests the global null throughout, so again, I don't think it makes much sense to say that it's a saturated model test ...} Define the saturated model as
\[
...
\[
\theta^E \;\;=\;\; \argmin_{\theta\in\R^{|E|}} \; \|\mu - X_E\theta\|_2 \;\;=\;\; (X_E'X_E)^{-1}X_E'\mu.
\]
...
\end{equation}
The pair $(E_{k-1}, j_k)$ defines the selection event and \smash{$\proj_{\eta_k}^\perp Y$} eliminates the nuisance parameter \smash{$\proj_{\eta_k}^\perp \mu$}. While the test statistic $X_{j_k}'Y$ is measurable with respect to $\sF_k$, the null distribution we compare it against is not: it depends on \smash{$\proj_{\eta_k}^\perp Y$}, which is not measurable with respect to $\sF_k$. As a result, saturated-model $p$-values constructed as in~\eqref{eq:satModel} are in general {\em not} independent on nulls.
...
\RJTcomment{I don't mean to sound overtly critical but I don't find this table helpful; in fact I find it confusing.  Why is the covariance test called a saturated model test?  Sure, it tests the complete null, but why does it use a saturated model framework?  I doesn't use the law at all as described in \eqref{eq:satModel}.  Also, the name ``spacings'' is reserved for a very specific approximation to the saturated-model test in LAR, that only uses knots in the LAR path, so I wouldn't use this name.  I think the last 3 subsections have explained the differences nicely, and I think this table doesn't really add much beyond them, and confuses me.  

Also, I don't know why we give the example below.  What does it show?  More examples follow in the next section, adn more detailed experiments follow later, and this just shows that the covariance test is conservative, but by now, we know this and have showed this many, many times over.  And it's not at all the point of this paper.}
...
We see that the next-entry test is the most powerful, especially at step 1. The next-entry and spacing test have the correct type I error in steps 3 and 4, while the covariance test becomes conservative in step 4, as we move further into the null part of the sequence.
\begin{figure}
...
\JTcomment{I would say our goal is a good stopping rule AND inference on parameters of selected (or perhaps saturated) model. We do not consider it here because we don't know how to do it, not because it is not interesting.}

\RJTcomment{I don't see why what you mention is a disadvantage of the saturated-model perspective.  In fact, I would argue that it is an advantage.  The model is always going to be misspecified.  What is being covered is a well-defined quantity, regardless of the true model.  That is precisely why I like the saturated-model perspective for confidence intervals...}
...
For lasso linear regression, the situation seems even more dire: for any fixed value of $\lambda$, the event \smash{$\{\supp(\hat\beta^\lambda) = E\}$} is a union of up to $2^{|E|}$ polytopes in $\R^n$, each satisfying $O(p)$ constraints \citep{lee2013exact}. \footnote{To finesse this issue, \citet{lee2013exact} propose conditioning on the signs of $\hat\beta^\lambda$ as well. If we do this, the conditioning set becomes a single polytope.}. But the event $\{E_{k-1}(Y)=E\}$ depends on $\supp(\hat\beta^\lambda)$ for every $\lambda > \lambda_k$ --- i.e., on the solution to {\em infinitely many} lasso problems.

\RJTcomment{I don't get this last paragraph really.  First of all, just to point out, for each of the forward stepwise, LAR, and lasso paths, we condition on the signs in our 2014 paper.  So this issue is not specific to the lasso.  Also, why are we talking about lasso for fixed $\lambda$ here?  This is confusing.  We have exact inference for steps along the lasso path (not the same problem as fixed $\lambda$ lasso) in our 2014 paper.  Our constructions give $\approx 2pk$ constraints for forward stepwise after $k$ steps, and $\approx 3pk$ constraints for LAR r lasso after $k$ steps.  Now, what is given below is still a big improvement over that!  But it's worth being very clear about the predecessors to it.}
