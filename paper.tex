...
\subsection{Sequential Sufficiency Principles}\label{sec:sufficiency}



\subsubsection{$p$-Values and the Sufficiency Principle}\label{sec:pvalSP}

A $p$-value for testing model $M_{k-1}$ against $M_k$ satisfies the sufficiency principle if it is a function only of the sufficient statistics of $M_k$
...
\begin{model}[Generalized Sparse Model]\label{mod:genSparse}
  ...
  \]
  For subsets $E\sub [p]$ define the model $M_E$ with active coefficients $E$ as follows:
  \[
  \Theta_E = \{\theta:\; \theta_j = 0, \;\;\forall j \notin E\}, 
  \quad M(E) = \{F_\theta:\; \theta\in \Theta_E\}.
  \]
\end{model}


In Model~\ref{mod:genSparse}, the {\em forward stepwise algorithm} proceeds as follows: we first set $E_0$ arbitrarily, then at step $k=1,\ldots,d$, we define
\begin{align}\label{eq:forwardDef_start}
j_k &= \argmax_j \max_{\theta\in\Theta_{E_{k-1}}} \ell(\theta; Y)\\
E_k &= E_{k-1} \cup \{j_k\}\\\label{eq:forwardDef_end}
M_k &= M(E_k)
\end{align}

\begin{proposition}[Forward Stepwise Satisfies SPSP]\label{prop:forwardSPSP}
  The forward stepwise algorithm defined in (\ref{eq:forwardDef_start}--\ref{eq:forwardDef_end}) satisfies the SPSP.
\end{proposition}
\begin{proof}
  \WFcomment{Write proof.}
\end{proof}
