...
\RTcomment{Will- be more specific here---   A parametric model  with specified nonzero support?}
...
\begin{proof}
  ...
  \RTcomment{Will; what are the qk?  you mean we just choose and fix some values qk? Say so.}Then, for the
  fixed hypotheses 
...
This is the procedure that used to obtain the p-values in Table \ref{tab:diab}  \RTcomment{ Rob- make sure you fix table 1 to make the correct computation}
...
\]
\RTcomment{Will- define $M_{[k-1]}$..  it is  the collection $\{ M_1 M_2 \ldots M_{k-1}\}$?}
...
\end{equation}
The next-$\lambda$ statistic is $\sF_k$-measurable if we use the LASSO to select variables, but not if we use forward-stepwise. \WFcomment{Relate this to spacings test. Is this essentially the ``generalized spacings test?''}
